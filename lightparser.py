from lightlexer import TOKEN_TYPE
from lightast import (
    new_var,
    new_func,
    new_struct,
    new_array,
...
def stmt(parser):
    ...
    elif curr == TOKEN_TYPE["Struct"]:
        return struct_stmt(parser)
...
def simple(parser):
    ...
    if kind == TOKEN_TYPE["Word"]:
        ...
        return left
    elif kind == TOKEN_TYPE["RightBracket"]:
        items = []
...
def struct_stmt(parser):
    parser.eat(TOKEN_TYPE["Struct"])
    id = parser.eat(TOKEN_TYPE["Word"])["value"]
    parser.eat(TOKEN_TYPE["LeftBrace"])
    keys = id_list(parser)
    parser.eat(TOKEN_TYPE["RightBrace"])
    return new_struct(id, keys)
